...
import { increaseQuantity, decreaseQuantity, deleteproduto } from "../../redux/OrebiSlice"; // Importe as ações corretas
  ...
  return (
    <div className="w-full grid grid-cols-5 items-center px-6 text-primeColor border-b py-4">
      <div className="flex items-center gap-4">
        ...
        <img className="w-20 h-20" src={produto.imagem} alt="Imagem do Produto" />
        <h1 className="text-sm font-titleFont font-semibold">{produto.nomeProduto}</h1>
      </div>
      ...
      <div className="flex items-center justify-center gap-4 text-lg">
        <span
          onClick={() => dispatch(decreaseQuantity({ idProduto: produto.idProduto }))}
        ...
        <span
          onClick={() => dispatch(increaseQuantity({ idProduto: produto.idProduto }))}
          className="w-6 h-6 bg-gray-100 text-xl flex items-center justify-center hover:bg-gray-300 cursor-pointer duration-300 border border-gray-300"
        >
          +
        </span>
      ...
      <p className="text-center font-bold">R$ {(produto.quantidade * produto.preco).toFixed(2)}</p>
    </div>
  );
};
